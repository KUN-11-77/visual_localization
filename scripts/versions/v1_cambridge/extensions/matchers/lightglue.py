...
class LightGlueMatcher(BaseMatcher):
    ...
    def match(self, query_data, db_data):
        ...
        data = {
            "image0": {
                "keypoints": torch.from_numpy(kpts0[np.newaxis]).float().to(self.device),
                "descriptors": torch.from_numpy(desc0[np.newaxis]).float().to(self.device),
                "image_size": torch.tensor([query_data.get("image_size", (0, 0))]).to(self.device),
            },
            "image1": {
                "keypoints": torch.from_numpy(kpts1[np.newaxis]).float().to(self.device),
                "descriptors": torch.from_numpy(desc1[np.newaxis]).float().to(self.device),
                "image_size": torch.tensor([db_data.get("image_size", (0, 0))]).to(self.device),
            },
        }
